...
class node:
    def __init__(self, quantities, parent):
        ...
        self.visited.add(quantities)
        self.quantities = quantities
        self.parent = parent
    
    def __str__(self):
        return 'quantities={}, has_parent={}'.format(self._quantities, self._parent!=None)
    
def visited(node, quantities):
    return tuple(quantities) in node.visited
        
def gen_solution_found(final_quantities):
    def solution_found(quantities):
        quantities_copy = list(quantities)
...
def find_toughest():
    ...
    max_steps = 0
    for i in tqdm(range(min(capacities[0], max_total_quantity), -1, -1)):
...
if __name__ == '__main__':
    solve_standard_problem(n_solutions=1)
    find_toughest()
    """
    capacities = [13, 7, 5]
